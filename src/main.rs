    ...
    fn new(id: usize, receiver: Arc<Mutex<mpsc::Receiver<Data>>>) -> Self {
        ...
        let thread = thread::spawn(move || loop {
            ...
            match corpus {
                Ok(corpus) => {
                    // let input = job();
                    let mut inp = corpus.choose(&mut rand::thread_rng()).unwrap().clone();
                    let inp_len = inp.len();
                    for _ in 0..rand::thread_rng().gen_range(0..32) {
                        inp[rand::thread_rng().gen_range(0..inp_len)] =
                            rand::thread_rng().gen_range(0..255);
                    }
                    let tmpfn = format!("tmp_{id:02}");
                    fs::write(&tmpfn[..], &inp[..]).unwrap();
                    ...
                    if output.status.success() {
                        count += 1;
                        println!(
                            "worker_{id:02}\ttime: {:#?}\tworker case: {count:010} | fcps: {:10.4}",
                            delta_t,
                            count as f64 / delta_t.as_secs() as f64
                        );
                    } else {
                        println!("Exited with status {}", output.status.code().unwrap());
                    };
                }
                Err(_) => {
                    break;
